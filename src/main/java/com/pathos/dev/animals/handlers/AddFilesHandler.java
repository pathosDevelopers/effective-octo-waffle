...
public class AddFilesHandler implements RequestStreamHandler {
    ...
    public void handleRequest(InputStream inputStream, OutputStream outputStream, Context context) throws IOException {
        ...
        JsonNode jsonFiles = json.get("files");

        List<String> files = intervention.getFiles();
        if(files == null) {
            files = new ArrayList<>();
        }

        List<String> finalFiles = files;
        jsonFiles.forEach(node -> finalFiles.add(node.get("file").asText()));
        intervention.setFiles(finalFiles);
